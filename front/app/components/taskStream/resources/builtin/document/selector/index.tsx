...
type DocumentResourceSelectorProps = {
  asElement?: boolean
  disabled?: boolean
  ...
  popupCls?: string
  readOnly?: boolean
  requiredFilterKey?: string
  trigger?: (open: boolean) => React.ReactNode
  triggerInnerClass?: string
  value?: string
}

const DocumentResourceSelector: FC<DocumentResourceSelectorProps> = ({
  asElement,
  disabled,
  offset = 6,
  onOpenChange,
  ...
  const filteredResourceList = allResourceList
    .filter(resourceItem => resourceItem.type === BuiltInResourceEnum.Document)
    .filter(resourceItem => !requiredFilterKey || !!resourceItem.data?.[requiredFilterKey])
  ...
  const defaultTriggerElement = (
    <Input
      placeholder={placeholder || '选择文档管理'}
...
export default memo(DocumentResourceSelector)
