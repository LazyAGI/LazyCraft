...
const nodeDefault: ExecutionNodeDefault<SubModuleNodeType> = {
  defaultValue: {
    ...
    payload__kind: 'SubGraph', // SubGraph App Wrap Loop
  ...
  checkValidity(payload: SubModuleNodeType) {
    ...
    const isWarp = payload.payload__kind === 'Warp'
    let validOverride = isLoop
    let errorInfo = ''
    ...
    if (isWarp) {
      if (!payload.payload__batch_flags || !payload.payload__batch_flags?.length || payload.payload__batch_flags?.every(flag => !flag)) {
        isValid = false
        errorInfo = '请至少设置一个批处理参数'
        checkFields.push({ name: 'payload__batch_params', error: errorInfo })
        validOverride = true
      }
    }
