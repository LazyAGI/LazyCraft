...
const useQuestionClassifierConfig = (nodeId: string, nodePayload: QuestionClassifierNodeType) => {
  const { nodesReadOnly: isReadOnly } = useReadonlyNodes()
  const { handleEdgeDeleteByDeleteBranch } = useLazyLLMEdgesInteractions()
  const { inputs: nodeInputs, setInputs: updateNodeInputs, handleFieldChange } = useNodeDataOperations<QuestionClassifierNodeType>(nodeId, nodePayload)
  ...
  const addClassificationCase = useCallback(() => {
  ...
  const removeClassificationCase = useCallback((caseId: string) => {
    const updatedInputs = produce(nodeInputs, (draft) => {
      if (draft.config__output_ports)
  ...
  const sortClassificationCases = useCallback((newCases: (ClassificationCase & { id: string })[]) => {
    const updatedInputs = produce(nodeInputs, (draft) => {
      draft.config__output_ports = branchNameValid([
  ...
  const updateCaseCode = useCallback((code: string, caseItem: any) => {
    const updatedInputs = produce(nodeInputs, (draft: any) => {
      const targetCase = draft.config__output_ports?.find((item: any) =>
        (item.id === caseItem.id),
      )
      if (targetCase)
  ...
  return {
    ...
    handleCreateCase: addClassificationCase,
    handleCodeChange: updateCaseCode,
    handlecurrentLanguageChange: updatecurrentLanguage,
    handleDeleteCase: removeClassificationCase,
    handleSortingCase: sortClassificationCases,
    handleFieldChange,
...
export default useQuestionClassifierConfig
