...
export function useWatch({
  ...
  const flattenedWatch = useMemo(() => formatWatch(watch as any), [JSON.stringify(watch || [])])
  currentWatchRef.current = useMemo(() => getWatchStatus({
    watch: flattenedWatch as any,
    nodeData,
    value,
  }, prevWatchRef.current), [JSON.stringify(flattenedWatch || []), JSON.stringify(nodeData || {}), value])
  ...
  }, [nodeId, JSON.stringify(flattenedWatch || []), JSON.stringify(nodeData || {}), value])
}
